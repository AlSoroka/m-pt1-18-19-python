# Александр Сорока
# Исходные данные для расчета, дигноза и рекомендаций взяты https://hudeyko.ru/raschet-imt.html

name=input   ("Введите Ваше имя: ")
while True:
	age =input   ("Введите Ваш возраст (число лет): ").strip()
	if age.replace('.','',1).isdigit() or age.replace(',','',1).isdigit(): break
...
while True:
	...
	if gender.lower() in ['м', 'ж']: break

while True:
	height=input ("Введите Ваш рост в сантиметрах (число в интервале от 100 до 250): ").strip() 
	# Если случайно ввели см
	height=height.replace('см.','').strip()
	height=height.replace('см','').strip()
	if (height.replace('.','',1).isdigit() or height.replace(',','',1).isdigit()) \
	and float(height)>100.0 and float(height)<250.0: 
		height=float(height)/100
		break

while True:
	weight=input ("Введите Ваш вес в кг (число в интервале от 10 до 250): ").strip() 
	# или кг
	weight=weight.replace('кг.','').strip()
	weight=weight.replace('кг','').strip()
	if weight.replace('.','',1).isdigit() or weight.replace(',','',1).isdigit() \
	and float(weight)>10.0 and float(weight)<250.0: 
		weight=float(weight)
		break
...
if imt<16.0:
	...
	advice='Усиленное питание, занятия ЛФК, сон не менее 8 часов, избегать \nстрессов и тяжелых физических нагрузок. Регулярный контроль веса'	
elif	imt<24.9:
	diagnosis='Номальный индекс массы'
	advice='Сбалансированное питание, занятия спортом, сон не менее 8 часов, \nизбегать стрессов'	
elif	imt<29.9:
	diagnosis='Лишний вес, предожирение'
	advice='Сбалансированное питание, больше двигаться, сон не менее 8 часов, \nизбегать стрессов. Регулярный контроль веса'				
elif	imt<34.9:
...
answer='\nУважаем'+'ый ' if gender.lower()=='м' else  'ая '
print(answer+name+"!")
...
for i in range(1, len(parts)):
	...
	# -1 - в ключевых точках ставим плюсы
	...
	len_curr=len(str(parts[i]))
	
	# Рассчитываем позицию первого знака числа:
	# Если это второе значение, то позиция '+' минус длина первого значения, 
	# если последующие, то позиция '+' минус половина длины предыдущего 
	# и последующего  значений
	
	dig_pos=plus_pos-len_prev+1 if i==1 else plus_pos-len_prev//2-len_curr//2+1
	dig_scl+=(dig_pos-len(dig_scl))*' '+str(parts[i])
	len_prev=len_curr
	j+=1
	
# рассчитываем экранную позицию полученного индекса
pos_imt=int((imt-parts[0])*div_val)

your='Ваше значение ИМТ в норме' if imt>=18.5 and imt<=24.9 else 'Ваше значение ИМТ в зоне риска'
metka=' '*(pos_imt-1)+'♥'+' '
res=''
