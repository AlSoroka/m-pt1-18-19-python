...
with open(os.path.join(path, fileIn), 'r') as fi:
    ...
    for line in fi:
        fieldValue = []
        separateField(line)
        # Функция возвращает строку разбитую  на поля в списке fieldValue

#  undefined fo и fieldCount определены ниже.
#  Блок выполняется только первого цикла, когда эти имена определены

        # Проверяем совпадает ли число столбцов данных с заголовком.
        if lineNum > 1 and fieldCount:
            if len(fieldValue) != fieldCount:
                print('В файле не совпадает количество столбцов данных \
                    с заголовками')
                if not fo.closed:
                    fo.seek(0)
